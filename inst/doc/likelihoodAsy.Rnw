...
practical route to validate the result of likelihood asymptotics. For example, we may use  10,000 bootstrap trials, 
...
The bootstrap-based $p$-value  agrees well with that based on $r^*$, as can be found by running the command 
...
0.1275 and 0.1240 respectively.
...
\bibitem{pierce2015} 
Pierce, D.A. and Bellio, R. (2015). Modern likelihood-frequentist inference. In preparation.
